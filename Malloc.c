/*	This file is part of the checking memory manager MALLOC.
	Written by Dick Grune, VU, Amsterdam; dick@dickgrune.com
	$Id: Malloc.c,v 1.32 2024-01-11 20:05:18 dick Exp $
*/
...
#define	alloc_bucket_for(x)	alloc_bucket[((intptr_t)(x)%HASH_SIZE)]
...
report_actual_call(FILE *out, const struct call *cl) {
	...
	if (cl->var_size) {
		...
		fprintf(out, "%s on average", any_uint2string(av));
		if (cl->n_blocks > 1) {
			fprintf(out, " = %s", any_uint2string(cl->size));
		}
	}
	else {
		...
		if (cl->n_blocks > 1) {
			...
			fprintf(out, " = %s", any_uint2string(all));
		}
	}
	fprintf(out, "\n");
}
...
void
Report_Memory_Status(FILE *out) {
	if (out == 0) out = stderr;
	report_actual_calls(out);

	fprintf(out, "Total memory allocated = %s", any_uint2string(total));
	fprintf(out, ", max. allocated = %s", any_uint2string(max));
	fprintf(out, ", still allocated = %s", any_uint2string(balance));
	fprintf(out, "\n");
	fflush(out);
}
...
void
Mem_Clobber(void *p, size_t size) {
...
_mreg_realloc(int chk, void *addr, size_t size, const char *fname, int l_nmb) {
	...
	if (old_size > 0 && size > old_size) {
		Mem_Clobber(((char *)res)+old_size, size-old_size);
	}
#endif	/* MEMCLOBBER */
...
#ifdef	MEMCLOBBER
	Mem_Clobber((char *)addr, old_size);
#endif	/* MEMCLOBBER */
	}
	free(addr);
}
